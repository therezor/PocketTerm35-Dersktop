...
fn parse(argv: &[&str]) -> Result<Request> {
    Ok(match argv {
        ["menu"] | ["menu", "toggle"] => Request::Menu { action: Toggle::Toggle, page: None },
        ["menu", "open"] => Request::Menu { action: Toggle::On, page: None },
        ["menu", "close"] => Request::Menu { action: Toggle::Off, page: None },
        ["menu", "open", page] => {
            Request::Menu { action: Toggle::On, page: Some((*page).to_string()) }
        }
        ["menu", page] => Request::Menu { action: Toggle::On, page: Some((*page).to_string()) },

        ["launch", app] => Request::Launch { app: (*app).to_string() },

        ["volume", value] => Request::Volume { change: delta(value)? },
        ["brightness", value] => Request::Brightness { change: delta(value)? },

        ["pointer"] | ["pointer", "toggle"] => Request::Pointer { mode: PointerMode::Toggle },
        ["pointer", "on"] | ["pointer", "move"] => Request::Pointer { mode: PointerMode::Move },
        ["pointer", "off"] => Request::Pointer { mode: PointerMode::Off },
        ["pointer", "grid"] => Request::Pointer { mode: PointerMode::Grid },

        ["scale"] | ["scale", "cycle"] => Request::Scale { value: None },
        ["scale", value] => Request::Scale {
            value: Some(value.parse().with_context(|| format!("bad scale {value:?}"))?),
        },
        ...
        ["touch", action] => Request::Touch { action: toggle(action)? },
...
fn print_status(s: &Status) {
    ...
    println!(
        "volume      {}{}",
        s.volume_percent.map(|p| format!("{p}%")).unwrap_or_else(|| "-".into()),
        if s.muted.unwrap_or(false) { " (muted)" } else { "" }
    );
    println!("network     {}", s.network.as_deref().unwrap_or("-"));
    println!("pointer     {}", if s.pointer_armed { "armed" } else { "off" });
}
...
mod tests {
    ...
    fn parses_the_bindings_used_in_the_sway_config() {
        assert_eq!(
            ...
            Request::Menu { action: Toggle::Toggle, page: None }
        );
        assert_eq!(parse(&["volume", "+5"]).unwrap(), Request::Volume { change: Delta::Relative(5) });
        assert_eq!(
            parse(&["brightness", "-10"]).unwrap(),
            Request::Brightness { change: Delta::Relative(-10) }
        );
        assert_eq!(parse(&["window", "fit"]).unwrap(), Request::WindowFit);
        assert_eq!(parse(&["pointer", "toggle"]).unwrap(), Request::Pointer { mode: PointerMode::Toggle });
        assert_eq!(parse(&["power", "menu"]).unwrap(), Request::Power { action: PowerAction::Menu });
        assert_eq!(parse(&["cpu", "balanced"]).unwrap(), Request::Cpu { profile: CpuProfile::Balanced });
        assert_eq!(parse(&["scale"]).unwrap(), Request::Scale { value: None });
        assert_eq!(parse(&["scale", "0.75"]).unwrap(), Request::Scale { value: Some(0.75) });
    }
